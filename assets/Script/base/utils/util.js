...
util.getCsvData = function(csvFile){
    ...
    cc.loader.loadRes(filePath, function(err,mapCsv){
        if(err){
            ...
            for(var lineIndex = dataBeginLine; lineIndex < mapArr.length; lineIndex++){
                ...
                for(var itemIndex = 1; itemIndex < itemArr.length; itemIndex++){
                    csvDataMap[lineIndex-dataBeginLine][mapKey[itemIndex-1]] = itemArr[itemIndex-1]
                }
        ...
        for(var i in csvDataMap ){
            ...
            if(csvDataMap[i].hasOwnProperty("ID")){
                nCount = nCount + 1
            }
